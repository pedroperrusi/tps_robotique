%%%%%%%%%%%%%% INIT %%%%%%%%%%%%%%

%close all
clear all
clc

rr=0.2; % robot_radius
rh=0.12; % head radius
l3=0.18; % link 3 length
n_points=100;

% head centre position wrt to Fb reference system
xT=0.57;yT=0; zT=0.57;pT=[xT,yT,zT];

%%%%%%%%%%%%%% TRANSFORMATIONS %%%%%%%%%%%%%%

% b->0
s0=pi/4;
C0=cos(s0);S0=sin(s0);
Rb0=[0,0,1;-1,0,0;0,-1,0]*[1,0,0;0,C0,-S0;0,S0,C0]; % rotation matrix
pb0=[xT;yT;zT]; % translation in the head centre
Tb0=[Rb0,pb0;0,0,0,1]; % transformation matrix
    
% joint 1
q1=pi/2*(1-2*rand(1));
C1=cos(q1);S1=sin(q1);
R01=[C1,-S1,0;S1,C1,0;0,0,1];  % rotation matrix
p01=[0;0;0]; % translation 
T01=[R01,p01;0,0,0,1]; % transformation matrix    
Tb1=Tb0*T01; 
    
% joint 2
q2=pi*(1-2*rand(1));
s1=pi/2;
C2=cos(q2);S2=sin(q2);
Cs1=cos(s1);Ss1=sin(s1);
R12=[C2,-S2,0;Cs1*S2,Cs1*C2,Ss1;-Ss1*S2,-Ss1*C2,Cs1];  % rotation matrix
p12=[0;0;0]; % translation 
T12=[R12,p12;0,0,0,1]; % transformation matrix    
Tb2=Tb1*T12; 

% joint 3
q3=0%rh*(1-2*rand(1));
s2=pi/3;
Cs2=cos(s2);Ss2=sin(s2);
R23=[1,0,0;0,Cs2,Ss2;0,-Ss2,Cs2];  % rotation matrix
p23=[0;q3*Ss2;q3*Cs2]; % translation 
T23=[R23,p23;0,0,0,1]; % transformation matrix    
Tb3=Tb2*T23;


%%%%%%%%%%%%%% FKM %%%%%%%%%%%%%%%%
D1=-S2*Ss2;
D2=q3*D1;
D3=Cs1*S2;
D4=Cs1*C2*Ss2+Ss1*Cs2;
D5=q3*D4;
D6=-Ss1*S2;
D7=-Ss2*Ss1*C2+Cs1*Cs2;
D8=q3*D7;
D9=C1*C2-S1*D3;
D10=C1*D1-S1*D4;
D11=C1*D2-S1*D5;
D12=S1*C2+C1*D3;
D13=S1*D1+C1*D4;
D14=S1*D2+C1*D5;

T03=[D9,D13*D6-D12*D7,D10,D11;D12,D7*D9-D10*D6,D13,D14;D6,D10*D12-D9*D13,D7,D8;0,0,0,1];
% numerical calculations to check
if max(T03-T01*T12*T23)<1e-8
     disp('Bravo/a')
end

r4=-rh;
x=D11+r4*D10; %x=px+r4*zx;
y=D14+r4*D13; %y=py+r4*zy;
z=D8+r4*D7;  %z=pz+r4*zz;

b_p04=Tb0*[x;y;z;1];
b_p04=b_p04(1:3)';
% numerical calculations to check
if (max(b_p04-[xT+(q3-rh)*Tb3(1,3),yT+(q3-rh)*Tb3(2,3),zT+(q3-rh)*Tb3(3,3)]))<1e-8
     disp('Tutto bene')
     disp(' ')
end

disp('*******************************')
disp('O4 position in Fb:')
disp(' ')
disp('  x=          y=       z=   ')
disp([b_p04(1) b_p04(2) b_p04(3)])
disp('*******************************')

%%%%%%%%%%%%%% IKM %%%%%%%%%%%%%%%%

disp('*******************************')
disp('q configuration:')
disp(' ')
disp('-> initial:')
disp(' ')
disp('  q1=          q2=       q3=   ')
disp([q1 q2 q3])


q3=rh-sqrt(x^2+y^2+z^2);

if q3==rh
    disp('Problem n.1')
else
C2=(Cs1*Cs2-z/(q3-rh))/(Ss1*Ss2);
    for i=[1,2]
        eps2=(-1)^i;
        S2=eps2*sqrt(1-C2^2);
        q2(i)=atan2(S2,C2);

        D1=-S2*Ss2;
        D4=Cs1*C2*Ss2+Ss1*Cs2;

        tmp_det=-(D1^2+D4^2);
        if tmp_det==0
            disp('Problem n.2')
        else
            tmp=1/tmp_det*[D4,-D1;-D1,-D4]*1/(q3-rh)*[x;y];
            q1(i)=atan2(tmp(1),tmp(2));
        end
    end
end

disp('-> IKM:')
disp('-> First solution:')
disp('  q1=          q2=       q3=   ')
disp([q1(1) q2(1) q3])
disp('-> Second solution:')
disp('  q1=          q2=       q3=   ')
disp([q1(2) q2(2) q3])
disp('*******************************')



% zoom to check (at the resolution)
 axis([xT+(q3-rh)*Tb3(1,3)-0.001, xT+(q3-rh)*Tb3(1,3)+0.001,...
     yT+(q3-rh)*Tb3(2,3)-0.001 yT+(q3-rh)*Tb3(2,3)+0.001,...
     zT+(q3-rh)*Tb3(3,3)-0.001 zT+(q3-rh)*Tb3(3,3)+0.001])

%%%%%%%%%%%%%% PLOTS %%%%%%%%%%%%%%
 
hold on 
axis([0 xT+1.5*rr,yT-1.5*rr,yT+1.5*rr,0,zT+2*rr])
xlabel('x');ylabel('y');zlabel('z')

line([0,xT],[0 yT],[0,zT],'LineWidth',2,'LineStyle','--','Color','k')

plot3(xT,yT,zT,'bo','MarkerSize',5, 'MarkerFaceColor','r')
plot3(xT-rr*Tb1(1,3),yT-rr*Tb1(2,3),zT-rr*Tb1(3,3),'co','MarkerSize',12, 'MarkerFaceColor','k')
plot3(xT-rr*Tb2(1,3),yT-rr*Tb2(2,3),zT-rr*Tb2(3,3),'co','MarkerSize',12, 'MarkerFaceColor','k')
plot3(xT-rr*Tb3(1,3),yT-rr*Tb3(2,3),zT-rr*Tb3(3,3),'co','MarkerSize',12, 'MarkerFaceColor','k') 
plot3(xT-rr*Tb3(1,3),yT-rr*Tb3(2,3),zT-rr*Tb3(3,3),'bo','MarkerSize',15)

circular_arc(pT,rr,0,s1,Tb1,'r-')
circular_arc(pT,rr,0,s2,Tb2,'y-')
line([xT+(q3-rh-l3)*Tb3(1,3) xT+(q3-rh)*Tb3(1,3)], ...
    [yT+(q3-rh-l3)*Tb3(2,3) yT+(q3-rh)*Tb3(2,3)], ...
    [zT+(q3-rh-l3)*Tb3(3,3) zT+(q3-rh)*Tb3(3,3)],...
    'Color',[0.6 0.6 0.6],'LineStyle','-','LineWidth',5)

colormap([1, 0.75, 0.79])
[xs,ys,zs]=sphere;

surf(xT+rh*xs,rh*ys,zT+rh*zs)

alpha(0.6)
shading interp


box on
grid on
view([0,-1,0]);
rotate3d on


